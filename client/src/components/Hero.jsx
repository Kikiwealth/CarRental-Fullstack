...
const Hero = () => {
    ...
    const {pickupDate, setPickupDate, returDate, setReturnDate, navigate} = useAppContext()

    const handleSearch = (e) => {
        e.preventDefault()
        navigate('/cars?pickupLocation= ' + pickupLocation + '&pickupDate=' + pickupDate + '&returnDate=' + returDate)
    }
  ...
  return (
    ...
    className='h-screen flex flex-col items-center justify-center gap-14 bg-light text-center'>
        ...
        onSubmit={handleSearch} className='flex flex-col md:flex-row items-start md:items-center justify-between p-6 rounded-lg md:rounded-full w-full max-w-80 md:max-w-200 bg-white shadow-[0px_8px_20px_rgba(0,0,0,0.1)]'>
            ...
            <div className='flex flex-col md:flex-row items-start md:items-center gap-10 min-md:ml-8'>
                <div className='flex flex-col items-start gap-2'>
                    <select required value={pickupLocation} onChange={(e) => setPickLocation(e.target.value)}>
                ...
                <div className='flex flex-col items-start gap-2'>
                    ...
                    <input value={returDate} onChange={e => setReturnDate(e.target.value)} type="date" id='pickup-date' className='text-sm text-gray-500' required/>
                </div>
                </div>
                     <motion.button 
        ...
        src={assets.main_car} alt="" className='max-h-74' />
